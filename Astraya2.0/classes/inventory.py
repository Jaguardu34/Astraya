...
class Inventory:
    ...
    def try_place_selected(
        self, current_map, tile_x, tile_y, block_grp, player_pos, entity_groups
    ):
        ...
        dist = (
            (player_pos[0] // 32 - tile_x) ** 2 + (player_pos[1] // 32 - tile_y) ** 2
        ) ** 0.5
        ...
        block_grp.add(
            ent.Block(
                texture.BLOCK_TEXTURE[item.place_biome_id],
                current_map,
                item,
                x=tile_x,
                y=tile_y,
    ...
    def __repr__(
        self,
    ):  # print(repr(player.inventory)) → affiche le contenu de l'inventaire ; (tu te doute qu'on ne peut pas afficher des classes sans les représenter d'une manière lisible, du coup on fait ça)
        lines = ["=== Inventaire ==="]
